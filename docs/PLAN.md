...
- **上傳順序：先檔後列**（先 upload 到 Storage，成功之後才 insert `media` 列）。反過來的話，上傳失敗會留下一列指向不存在檔案的 `media`——時間軸出現破圖，而清掉它需要 media 的寫入權；照這個順序失敗則只留下一個孤兒物件，而 policy 明確允許上傳者自己刪掉它。**代價要知道**：`families.storage_used_bytes` 是 `media` 的 trigger 依 `byte_size` 維護的，所以**額度只在 media 列這一層綁得住**——檔案已經進 bucket 了才輪到額度判斷，Storage 的實際 bytes 一定 ≥ 帳面值。這不是可以靠調順序解決的問題（反過來就變成破圖），只能靠**離線對帳**收斂：以 `media.storage_path` ↔ `storage.objects.name` 為基準，`media` 沒有對應列的物件即孤兒。縮圖路徑自 LS-128 起是 `media.thumb_path` 欄位本身（此段原本寫「縮圖不在 `media` 裡，推導規則是 `regexp_replace(...)`」，是 LS-128 之前的舊描述，已過時）；**`storage_used_bytes` 不含縮圖大小**（`byte_size` 只計原始檔案），所以對帳時要把縮圖物件從「孤兒」判定裡排除，也要知道帳面值天生低估。`media` 沒有對應列的物件除了 client 端 best-effort 清理（見 `docs/API.md` §3「上傳中斷時的三種不一致收斂」，LS-212）之外，`purge-storage` Edge Function（LS-213）每次執行會分頁掃描 `media` bucket 反查 `public.media.storage_path`／`thumb_path`，把超過寬限期（24 小時）仍找不到對應列的物件排入既有的 `purge_storage_queue` 一併刪除——見 `docs/API.md` §6「自動清除」；掃描本身有每次 invocation 的物件數上限（`ORPHAN_SCAN_BATCH_SIZE`），物件量體極大或排程未接上時仍可能暫時累積，帳面值仍可能短暫低估。
...
**Storage 的雲端部署驗證清單**（本機測不到、只在雲端才成立的三件事；`db push` 綠燈不能取代這三條；三條已機械化為 `scripts/ops/prod-storage-verify.sh`，LS-244）：
