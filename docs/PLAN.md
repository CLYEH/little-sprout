...
- **審核用 demo 帳號** —— 邀請制 app 的死穴：審核員登入後沒有家庭、沒有邀請碼，看不到任何功能會被判「無法審核」而退件。須提供可用帳號 + **已有孩子檔案、照片、日記、留言的示範家庭** + 長期有效邀請碼。另外 Sign in with Apple 對審核員偶爾不順，保留 Email OTP 給他們（原本就規劃了，正好）。**OTP 收信方案已裁決採方案 C**：審核帳號用一個審核員真的能登入的真實信箱收驗證碼，密碼只放 App Store Connect Review Notes、不進 repo（見 `docs/store/review-notes.md`）
