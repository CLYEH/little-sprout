...
- **留言**：LS-241 起日記詳情頁與時間軸卡片共用同一套互動列（`InteractionRow`，
  LS-216）與留言 sheet（`CommentsSheetView`，LS-218），不再是預留位置——**種子腳本
  建的 4 則留言／4 個愛心**會直接顯示（愛心計數、按讚名單、留言清單皆可互動）；審核
  員點進日記詳情看到的留言計數與時間軸卡片一致。

**送審前待辦**：若「相簿」在送審當下仍是 placeholder，審核備註應該明講「這個區塊的
畫面正在開發中」，避免審核員誤判成 app 半成品而以 Guideline 4.2（最低功能性）退件
——這點需要送審前再次確認這張票的實際進度（相簿：LS-126 提到的後續票）；「留言」
已在 LS-216／LS-218／LS-241 落地，不再是 placeholder。
