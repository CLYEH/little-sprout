...
- **Release**：`bash scripts/ops/promote.sh test main`（FF push）後打 tag `vX.Y.Z`（semver），TestFlight 上傳由 orchestrator 執行並在對應 ticket 記錄 build 號。
