...
- **Hotfix**：`hotfix/*` 從 `main` 切出、PR 回 `main`（CI＋review 照跑、QA 走快速通道），合併後**只補一支 back-merge `main`→`development`**（`gh pr create --head main --base development`；LS-85 起唯一保留的 back-merge），`test` 不補——下次 `promote.sh development test` 帶到。
...
| 晉升 development→test、test→main 一律 fast-forward push（`promote.sh`），不開 PR、不留 merge commit、禁手動推 test／main（§2，LS-85） | `scripts/ops/promote.sh`：方向白名單（其餘 exit 2）、`git fetch`、`merge-base --is-ancestor origin/<to> origin/<from>` 非 FF 即拒並指示 back-merge、`gh api …/commits/<sha>/check-runs` 四 job 各取最新一筆（id 最大）須 completed＋success（缺／skipped／failure／in_progress 皆拒並點名）、`PROMOTE_VIA_SCRIPT=1 git push origin origin/<from>:refs/heads/<to>`；push-gate 第 0 步 `scripts/gates/push-ref-check.sh`：目標 refs/heads/test\|main 無 `PROMOTE_VIA_SCRIPT=1` 或非 FF（remote sha 非 local sha 祖先；remote sha 本機沒有＝沒 fetch，fail closed）即擋；GitHub 分支保護：test／main 關閉 require PR、required checks 對推上去的 SHA 生效（沒綠 check 的 SHA 推不上去，GH006）、enforce_admins、禁 force-push（`scripts/ops/protection-apply.sh`，前後 JSON 存證）；自測 `promote.test.sh`（13 組：方向錯／非 FF／check 紅／缺／skipped／未完成／同名多筆取最新／正常放行且 hook 收到 PROMOTE_VIA_SCRIPT=1／已最新／gh 失敗／遠端拒收／不在 repo）與 `push-ref-check.test.sh`（16 組＋經 push-gate.sh 接線 4 項）掛 CI `rules` job。**盲區**：GitHub 沒有「拒絕非 FF 推送」的規則（require linear history 會連 development 自己的 PR merge commit 一起拒），`--no-verify` 繞過 push-gate 後 server-side 只擋「沒綠 check 的 SHA」、擋不了帶綠 check 的非 FF merge commit——靠下列「分支漂移」巡檢偵測＋規約 | ✅ hook＋GitHub（非 FF 僅客戶端；漂移⚠️巡檢） |
