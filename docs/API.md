...
- **帳號停權／刪除過渡期**：停權（LS052／LS053）同其餘「自著內容」表，見 §2、§5。
  **已申請刪帳（LS051）跟 `growth_records` 不同**：`child_food_records_deletion_
  guard` 一樣只掛 `BEFORE INSERT`，但 `upsert_child_food_record` 的更新分支是
  `INSERT ... ON CONFLICT DO UPDATE`，trigger 對提議列求值、衝突與否都會觸發
  （理由同上面 LS044 那條）——已申請刪帳的作者呼叫 `upsert_child_food_record`
  編輯既有紀錄一樣會撞 `LS051`，不像 `growth_records` 的更新走純 `UPDATE` 陳述式
  不會重新觸發 `BEFORE INSERT`、過渡期仍可編輯既有紀錄。見 §5 `LS051` 列。
...
- **相簿列表請讀 `album_summaries`（LS-200），不要用 client 端內嵌 aggregate 算張數**：
  `album_summaries` 是一支 `security_invoker=true` 的 view，逐本相簿多帶六個彙總欄——
  `visible_media_count`（呼叫者依 RLS 看得到的照片數）、`latest_media_id`／
  `latest_thumb_path`／`latest_storage_path`（依 `created_at` 排序、可見範圍內最新
  一張的 media id／縮圖路徑／原圖路徑）、`cover_thumb_path`／`cover_storage_path`
  （`cover_media_id` 指到的那張縮圖／原圖路徑，若該張已軟刪／跨家庭則皆為
  `NULL`）。`*_storage_path` 兩欄是 `*_thumb_path` 為 `NULL` 時（既有資料、縮圖
  產生失敗的過渡列）的退回來源——沿用上方「簽名 URL 與 egress 防線」段落既有的
  `thumb_path` 為 `NULL` 時退回 `storage_path` 規則，只是把退回會用到的兩個路徑
  一次給齊，不必呼叫端另外反查 `media` 表。`security_invoker=true` 讓 view 內部對
  `albums`／`album_media`／`media` 的存取套用呼叫者本人的 RLS
  ...
  **view 欄位於建立時凍結（R2 N3）**：`select a.*, ...` 這種寫法的 `a.*` 是在
  ...
  `album_summaries` 裡，也不會有任何錯誤或警告提醒這件事，只會在呼叫端納悶
  「明明 `albums` 有這欄，為什麼 view 讀不到」時才會發現。幫 `albums` 加欄的
  migration 若也想讓 `album_summaries` 一起看到新欄位，必須額外下一次
  `CREATE OR REPLACE VIEW public.album_summaries AS ...`（同一份 SQL 文字，只是
...
- `reactions`（LS-58）：加入／收回都收斂進 `toggle_reaction` RPC（見 §4），直接
  `.insert()`／`.delete()` 一律 `42501`。**封鎖過濾（LS-225）**：`reactions_select`
  RLS policy 疊加了 `blocked_pairs` 述詞，被封鎖者的反應在直接 `SELECT` 這張表時
  看不到，細節見下方「封鎖過濾」小節與 §4 `get_reaction_counts` 的對應說明。呼叫端
  **不再需要**自己處理
...
- **封鎖過濾（LS-149；reactions 由 LS-225 補上）**：被封鎖者的內容在下列查詢一律
  看不到——`albums_select`／`comments_select`／`reactions_select` 三條 RLS policy、
  `get_family_timeline` RPC（時間軸）、`get_reaction_counts` RPC 都疊加了
  ...
  直接讀取路徑不在 LS-149 票文列的測項內，是刻意縮小的範圍。

  | 查詢／表 | 過濾方式 | 補上版本 |
  | --- | --- | --- |
  | `albums_select` | RLS policy `NOT EXISTS` | LS-149 |
  | `comments_select` | RLS policy `NOT EXISTS` | LS-149 |
...
- **語意**：**全覆蓋**（PUT，不是逐一新增／移除）——`p_child_ids` 為 `NULL` 或
  ...
  `p_child_ids` 語意，見 §8）。**刻意不檢查照片是否已軟刪除**（同 `set_album_
  children` 的既定行為，見上）——已軟刪除的照片仍可改標記，這是「內容標記」這一
  支動作既有的既定行為，不是通用規則。
- **`deleted_at`／還原期間的行為**（同 `set_album_children` 的 LS-121 R2 行為，
  見上）：軟刪期間改標記，`media_children` 照樣落地，但 `feed_item_children`
  （`get_family_timeline` 篩 child 用的查詢引擎，見 §8）在軟刪期間維持 0 列
  （該照片本來就不在 `feed_items` 裡）；還原後 `feed_item_children` 會依
  `media_children` 當下（軟刪期間可能已經改過）的集合重新展開，不是還原成軟刪前
  的舊集合。
...
- **用途**：批次設定多張照片的寶貝標記，供相機膠卷批次匯入一次多張使用（見
  ...
  ["<uuid>", ...]}]`，票面效能要求「200 筆 <1s」（實測 200 筆 35ms）；**上限
  500 筆，超過拋 `22023`**（merge-review R1 i1：呼叫端若一次送幾千筆，會是一個
  長交易持有同等數量的 media 列鎖，500 是票面驗收批次量 200 的 2.5 倍，留足匯入
  分批的彈性同時避免無上限）。
- **鎖序**：批次內部依 `media_id` 遞增排序後才逐筆取鎖，不論呼叫端給定的陣列
  順序為何——兩個順序相反的重疊批次因此只會排隊等待，不會構成循環等待
  （merge-review R1 m1：R1 版本依呼叫端陣列順序取鎖，已實測兩個順序相反的重疊
  批次會 `40P01`）。同一 `media_id` 在單一批次內重複出現時仍是「陣列原始序號
  較後者勝」。排序鍵自 LS-320 起轉型為 `::uuid`（原為呼叫端送來的裸字串）——
  純內部實作細節，讓「所有呼叫端對同一組 media 得到同一個全序」這個保證與
  資料庫 collation 無關（merge-review R2 `a63b98e8` i6：裸字串排序在
  `en_US.UTF-8` 下與 uuid 排序恰好一致、`C`／`POSIX` collation 下會分歧）；
  呼叫端行為、簽章、錯誤碼、原子性皆未變。
...
- **封鎖過濾（LS-225）**：`reaction_count` 排除呼叫者已封鎖的人（`blocked_pairs`
  述詞，同 `albums_select`／`comments_select`／`get_family_timeline` 既有寫法，見上方
  §3「封鎖過濾」小節），單向——只影響封鎖者自己看到的計數，被封鎖者與其他成員的
  視角不受影響。`reacted_by_me` 不受這個述詞影響（呼叫者不可能封鎖自己）。
...
- **`p_child_id`**：`NULL`＝不篩（回傳全部，含沒有任何孩子標記的項目與所有
  ...
  查詢路徑改走 `feed_item_children`（見 §8 的設計取捨）。**`media` 類項目自 LS-317
  起與 `diary`／`album` 同一套語意**——被 `set_media_children` 標記過該孩子的照片
  會出現；沒標記過的（含只透過相簿／日記間接關聯的）不出現，見 §3 `feed_items` 段。
- **回傳的 `child_ids`**：該項目標記的**全部**孩子（不只是 `p_child_id` 篩選命中的
  ...
  不需要另外查一次。沒有任何標記時是空陣列 `{}`，不是 `NULL`（未標記過的 `media`
  即此）。
