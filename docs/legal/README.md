...
| 推播通知（隱私 §2.5） | **LS-217**（客戶端授權＋裝置 token 註冊）；`push-dispatch` 發送端待 **LS-8**（APNs 正式金鑰） | **已上線（裝置 token 蒐集面）**：LS-217 併入後，App 在使用者允許通知後呼叫 `registerForRemoteNotifications()` 並以 `register_device_token` RPC 把 device token 存進 `device_tokens`（`LittleSprout/Services/Push/PushNotificationStore.swift`／`PushAuthorizationService.swift`／`PushDeviceTokenAPIClient.swift`；`PushPrepromptView` 為登入後全屏 gate，QADriver 已標記）；`delete_my_account()`／LS-151 tombstone finalize 會刪除該筆 `device_tokens`（`docs/API.md` 「Tombstone 時一併硬刪」段）。**發送端尚未上線**：`supabase/functions/push-dispatch` 已建但 `docs/API.md` §「push-dispatch」明記「不含 APNs 正式金鑰與正式站部署（待 LS-8）」、且未接排程，目前沒有任何東西會觸發它。正文 §2.5（LS-132 R3）已改為反映「識別碼已蒐集、發送分階段上線」的現況陳述，屬個資法告知事項之實質內容變更，**待使用者核可** |
