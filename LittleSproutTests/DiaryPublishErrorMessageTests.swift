...
final class DiaryPublishErrorMessageTests: XCTestCase {
    ...
    /// 因此是兩個碼、兩句文案——這條釘住「螢幕上出現的是裁短那句、而且帶得出建議秒數」，
    /// mutation 若把新分支刪掉會退回 413 之外的通用句而紅。
    func test_videoTooLargeAfterExport_showsTrimToSecondsMessage() {
        let error = AppError.validationRetryable(
            message: "log only, not shown", code: DiaryMediaErrorCode.videoTooLargeAfterExport
        )

        let text = DiaryPublishErrorMessage.displayText(for: error)

        XCTAssertEqual(text, "影片太長，壓縮後仍超過 50MB 上限，請裁到 40 秒內再試一次。")
        XCTAssertTrue(
            text.contains("\(MediaUploadLimits.suggestedVideoSeconds) 秒"), "要告訴使用者裁到幾秒，不是只說太大"
        )
    }
