...
/// `WelcomeView` 三顆登入鈕互斥邏輯的測試（R1 review F1／I2、R2 review F1-A，PR #163）。
///
...
/// 之後這條判定本身有測試釘住。
///
/// F1-A：R2 review 指出 F1 的 `.disabled()` 對官方 `SignInWithAppleButton` 很可能是
/// no-op（見 `AuthButtonsState.shouldAcceptAppleCompletion` 檔頭說明），view 層的 disable
/// 不能當互斥的唯一防線。這裡直接測 completion 入口的 model 層守門純函式。
///
...
final class AuthButtonsStateTests: XCTestCase {
    ...
    // MARK: - F1-A：Apple completion 入口的 model 層守門（R2 review，`.disabled()` 對官方鈕可能 no-op）

    func test_shouldAcceptAppleCompletion_rejectsWhenGoogleInFlight() {
        XCTAssertFalse(
            AuthButtonsState.shouldAcceptAppleCompletion(isSigningInWithGoogle: true, isNavigatingToEmail: false),
            "Google 在跑時 Apple 的 completion 必須拒收，否則兩條登入流程能並行寫入 AuthStore.session"
        )
    }

    func test_shouldAcceptAppleCompletion_rejectsWhenNavigatingToEmail() {
        XCTAssertFalse(
            AuthButtonsState.shouldAcceptAppleCompletion(isSigningInWithGoogle: false, isNavigatingToEmail: true),
            "已導向 Email 登入流程時 Apple 的 completion 也必須拒收"
        )
    }

    func test_shouldAcceptAppleCompletion_acceptsWhenNeitherInFlight() {
        XCTAssertTrue(
            AuthButtonsState.shouldAcceptAppleCompletion(isSigningInWithGoogle: false, isNavigatingToEmail: false),
            "沒有別的登入方式在跑時，Apple 的 completion 要正常被接受"
        )
    }
}
