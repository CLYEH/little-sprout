...
--
-- 方向 A（編輯先動）測不到 set_child_deleted 自己那把 `for update`：先動的那一邊反正
-- 會在自己的 UPDATE 上取得列鎖，後動的一邊只要有鎖就會排隊。要讓 set_child_deleted
-- 的鎖成為唯一的防線，必須讓「軟刪」當先動的那一個——這是這個方向存在的理由，同
-- diary_edit_vs_delete 兩個方向缺一不可（mutation test 的教訓：只跑單一方向，拿掉
-- 其中一支 RPC 的鎖測試仍然是綠的，因為先動的那邊反正會在自己的 UPDATE 上取鎖）。
--
