...
--    正規化（去除非英數、轉大寫）之後直接對 invites.code 做「完全比對」，交給既有的
--    三段檢查（碼存在？未過期？次數未用完？）分流。這對 8 碼舊格式碼會有兩種結果，
--    取決於那支碼原本是不是真的存在過（R1 F1 訂正——第 3 段是 UPDATE 不是 DELETE，
--    列還在）：
--      - 曾經真實存在、下面第 3 段的 UPDATE 已把它標記過期的那支列：request_join
--        找得到列（code 完全比對成功），但 expires_at 已經是過去，落在既有的
--        「已過期」檢查 → LS011（邀請碼已過期）。
--      - 從來沒被 create_invite 產生過、單純打錯或亂猜的字串：查無此列 →
--        既有的 LS010（邀請碼不存在）。
--    兩種情況都不需要、也不應該，額外加一層專屬的長度／格式檢查（多一個要跟字元集
--    規則保持同步的地方，見 supabase/tests/80_join_approval.sql 新增的兩個對照案例）。
--
