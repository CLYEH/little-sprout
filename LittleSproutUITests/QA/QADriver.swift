...
final class QADriver {
    ...
    /// LS-217 QA R1 FAIL（Linear comment `e4863482`）修正：`PushPrepromptView`「稍後再說」
    /// 按鈕——用 `QAAccessibilityID.pushPrepromptSkipButton` identifier 而非明碼（見該常數文件
    /// 註解），`dismissPushPrepromptIfPresent()` 靠它判定登入後首次進時間軸的推播前置頁是否
    /// 出現。
    var pushPrepromptSkipButton: XCUIElement { app.buttons[QAAccessibilityID.pushPrepromptSkipButton] }
    ...
    /// LS-217：登入後首次進時間軸自動彈出的推播前置頁（見 `dismissPushPrepromptIfPresent`）。
    ...
    /// LS-217 QA R1 FAIL（Linear comment `e4863482`）修正：`AuthenticatedRootView` 的
    /// `.task(id:)`（`Navigation/RootView.swift:143-146`）非同步查完系統推播授權狀態後才決定
    /// 要不要蓋上 `PushPrepromptView`（`:147-158`）——這個時機點在上面 `require(timelineHeading,
    /// …)` 已經看過一次時間軸「之後」，`openDiaryEditor()`／`browseDetailAndAlbums()` 接著點的
    /// 元素會被這個新蓋上來的 `.fullScreenCover` 擋住而撲空（同 `qa-e2e.sh browse`／`publish`
    /// 對全新帳號必現的根因，QA 證據 `.claude/evidence/LS-217/qa-e2e/browse-20260912-151026/`）。
    /// 同 EULA 前例（`assertLandedAfterLogin` 命中「EULA 同意頁」的處理）：判定元素出現就點掉、
    /// 再等一次時間軸；`.notDetermined` 以外的授權狀態或這個帳號已經看過都不會出現，此時
...
// LS-220 起「登入落點失敗診斷」（`landingCandidates`／`waitForAny(_:_:timeout:failureNote:
// hierarchyReason:)`／`closestScreenTitles`）已搬到 `QADriver+LoginLanding.swift`（LS-217 QA R1
// FAIL 修正：本檔加推播前置頁判定後超過 SwiftLint `file_length`，見該檔檔頭註解）。
