...
- **最小像素**：≥ **2160×1190px**（iPad 顯示區 1079×594.5 的 2 倍，取六板中最大者；iPhone 690×380.2 的 2 倍僅 1380×761，已被 iPad 需求涵蓋，故以 iPad 為準）。**建議實際產出 2400×1322px 以上**（同比例，留裁切安全邊界，見下）。
- **若生圖工具僅提供固定預設比例**（例如 1536×1024／1024×1536／1024×1024 三選一）：選最接近的橫幅選項（1536×1024，比例 1.5）並取最高可用解析度，構圖時**主體與關鍵動作（擁抱、親吻額頭、十指相扣）需落在畫面水平置中 70% 範圍內**，讓後製可以從 1536×1024 中央裁切出 1536×846（比例 1.816，符合目標）而不切到臉部或手部；裁切後需確認仍 ≥ 2160×1190（如否，需以更高解析度重新生成或做無損放大）。
...
5. **與印品白邊的邊界融合度**（機械，色差）：計算照片四角 32×32px 區塊平均色與 `$print-paper` #FBEBEC 的 CIE ΔE，須 > 15（避免照片邊緣色調太接近白邊、讓「這是一張獨立相片」的邊界感消失，與角托 corner-out 5pt 的視覺切割一起被稀釋）。
