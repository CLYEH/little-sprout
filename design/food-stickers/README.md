...
版控內的 274 張成品是用腳本檔頭釘死的 **Pillow==10.3.0** 產出（LS-338 merge-review m2：不同 Pillow 版本像素
不變但 PNG 編碼層會變，整批無視覺差異的 churn）。**釘版本不保證跨機器逐位元組重放**——LS-340 實測：同一台
機器、同一個 Pillow 版本，光是「這次重切」與「版控內既有 blob 是哪台機器產出的」不同，`cmp` 逐位元組比對就
可能不同（像素透過 `Image.tobytes()` 解碼仍完全相同）。所以重切前先對齊版本能避免「換版本造成的」churn，但
不要預期能單靠這個做到跨機器位元級重放；真的要驗證重切正確與否，看像素內容是否相同，不要看檔案是否
`cmp` 相同。
