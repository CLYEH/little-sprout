...
extension TapTargetGateHarness {
    ...
    // LS-246 R2：`diaryDetailWithVideoHost` 搬到 `TapTargetGateHarness+DiaryDetailVideo.swift`
    // ——見該檔文件註解（R1 版本用假網路 URL 觸發了 dev CI 上 AVKit 的自動收起行為，R2 改成
    // 本機合成真實可播放檔案，需要額外的 `AVAssetWriter` 邏輯，疊在這裡會超過 file_length）。
}
#endif
