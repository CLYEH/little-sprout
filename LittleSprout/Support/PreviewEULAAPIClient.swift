...
extension EULAStore {
    ...
    /// ——harness 需要免登入、無時序窗口就能直接渲染出畫面。
    @MainActor
    static func preview(shouldPresent: Bool) -> EULAStore {
        let store = EULAStore(apiClient: PreviewEULAAPIClient())
        store.seedForPreview(shouldPresent: shouldPresent, currentVersion: "2026-09-05-draft")
        return store
    }
}
#endif
