...
final class FamilyStore {
    ...
    var createInviteState: FamilyOperationState = .idle
    ...
    ///
    /// merge-review R2 B3：`lookupInviteState`／`latestInvite`／`createInviteState`（上方）
    /// 不再是 `private(set)`——三者對應的方法拆去 `FamilyStore+Invite.swift`（這支檔案跟
    /// `SettingsView.swift` 同一天各自被兩張票疊加壓到 `file_length` 400 行上限，這裡拉開
    /// 餘裕，理由同 `FamilyStore+JoinRequests.swift` 檔頭既有先例）。純搬移，不改行為。
    var lookupInviteState: FamilyOperationState = .idle
    var latestInvite: GeneratedInvite?
    ...
    /// LS-190：多帶 `ownerUserID` 讓 `syncOwner(to:)` 短路、不洗掉這裡種好的值，同時補
    /// `lookupState = .success` 避免 `familyGate` 卡在查詢中（`eulaConsentToTimelineHost` 用）。
