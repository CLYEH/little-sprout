...
| `paper-rule`（LS-119 R3） | #5530404D | #5530404D | 紙上的分隔線（Card Diary Sign-off Rule）：`$print-ink-secondary` 25% alpha，不掛 theme——躺在紙上的線不該跟著 theme 反轉（同 print-ink 系列邏輯）。取代誤用 `$border`（theme-aware）疊在 `$print-paper`（不掛 theme）上導致深色對比暴衝 3 倍的 bug | 淺 2.29:1／深 6.83:1 兩者統一後不再適用（新 token 本身無 theme 差異） |
...
| `fs-otp` | 36 | 52 | OTP 六格數字（等寬）；也是邀請碼輸入格（6 碼 3+3）的字級。已擱置備案：若升八格則 AX3 52→42（`h7EnT` R6 ②，見 motifs.md 邀請碼） | `.system(size:).monospacedDigit()` |
| `fs-code` | 60 | 72（Tokens 板格子 `F7Swz6`＝變數 `$fs-code` ax3＝72，LS-81 已對齊；`h7EnT`④「六位 AX3 76 會溢出 8.6，已降 72」） | 邀請碼大字（等寬，6 碼 3+3 分組）。已擱置備案：若升八格則 60→52、AX3 ≤52（`h7EnT` R6 ④，見 motifs.md 邀請碼） | `.system(size:).monospacedDigit()` |
