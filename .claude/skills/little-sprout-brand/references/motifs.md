...
- Pencil 實作細節（旋轉 bounds 訂正證據、preShift／padding-top 逐板精算值與踩雷記錄、MJ-6 標題對照表、MJ-7 深色板限制記錄、MJ-8～BL-10 處置、Tab-root 通用判準與 `a67Na` 範例補記）見 `design/littlesprout.pen` Handoff Notes `LuHbv`（LS-120／LS-120 R2／LS-120 R3／LS-120 R4／LS-120 R5／LS-120 R6～R8 段；**R3 段的板高延伸數字、R4 段的旋轉包絡公式與逐板 padding-top 舊數值均已標記作廢，勿依其實作，以 R5 段起（含 R6～R8 對 `HLXo3` 的後續修正）為準**）。
