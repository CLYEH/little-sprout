...
- **y 位移（裝置固定值，R4 起不再有例外）**：Tab-root 板既有公式「y＝板高−98」（34pt 底部安全邊界＋64pt 膠囊高）／AX3「y＝板高−122」（34＋88），板高固定 852／1500（見下），因此**全 app 所有 Tab-root 板的膠囊 y 只有兩個值：754（預設）／1378（AX3）**，不因畫面內容量、狀態（有內容/深色/下拉中/空狀態）而變動。
...
- **驗收判準（R4，取代 R2「0 重疊」與 R3「entirely above/past」兩版模糊講法）**：兩條，缺一不可——①板內任何葉節點（文字／icon，含旋轉節點的旋轉後包絡框，見下）與膠囊矩形零垂直交集（不得被膠囊上緣切一半）②膠囊下緣到板底的 34pt home indicator 帶內不得有任何節點（不得被擠進那條帶、也不得被板底切一半）。等價講法：內容要嘛完全在膠囊頂之上，要嘛完全在板底之外（被 `clip` 裁掉），膠囊和板底之間不存在合法的內容位置。
- **旋轉感知（R4 新增，BL-5）**：`cmp/Day Divider` 的 `Stamp`（rotation −6°）／`Ghost`（−3°）兩個節點的 `ctx.bounds` 是**未旋轉**的區域框，沿 parentCtx 鏈單純累加 x/y 對這兩個節點以下的任何內容都算錯——旋轉是渲染時另外套用的視覺變換，不影響 layout 算出的 x/y/w/h。判準：對自身 `rotation≠0` 的節點，取其**中心點**（`x+w/2, y+h/2`）與旋轉後包絡框（`Hp=w·|sinθ|+h·|cosθ|`），範圍＝`[中心−Hp/2, 中心+Hp/2]`，取代其原始 AABB，且不再往下展開子節點（用整顆 Stamp／Ghost 的包絡框當保守上界，不追內部文字的精確旋轉座標）。**校準證據**：`HLXo3` 的 `kGF7O/xlLIo`（Stamp，−6°，local w=335.51／h=104.11）代入本公式得 Y 範圍 1439.60..1578.21，與獨立覆核算出的 1439.6..1578.2 完全吻合（差距 <0.01pt）——**確認 Pencil 對 `rotation` 是繞節點自身包絡框的「中心點」旋轉，不是 execute.md 文件字面寫的「左上角」**（文件與實作不一致，比照既有 width/height `$variable` 落空的性質記錄為工具限制）。
- **修法：兩個內容側旋鈕，皆為 padding／gap，不是板空間**（R4）：①**preShift**——3 板（`yXSht`／`a67Na`／`HLXo3`）把 Feed 之前既有的 `Spacer Section`（原 44）降到 0，讓 Feed 整體上移 44，清空 Day Group 1 最後一則內容與膠囊的交集；另 3 板（`V4Ktak`／`A33la`／`lKoZG`）Day Group 1 本來就不重疊，preShift＝0。②**Day Group 2 的 `padding-top`**——6 板各自在自己的 `Day Group 2` 節點加 `padding:[X,0,0,0]`，把 Day Group 2（含旋轉的 stamp/ghost）整段推到板底之外：yXSht 48／a67Na 71／V4Ktak 112／A33la 112／lKoZG 114／HLXo3 108（精算值＋3.5~4.5pt margin，公式與逐板數字見 Notes `LuHbv` R4 段，非拍腦袋數字）。**踩雷記錄**：第一次嘗試改 `Feed` 自己的 `gap`（$sp-section 44→G）只想推 Day Group 2，但 `gap` 是 Feed 對全部子節點統一套用的屬性——`a67Na` 在 Day Group 1 之前多一個 `Pull Refresh Row`，同一個 gap 改動連它與 Day Group 1 的間距也一起變了，反而把 Day Group 1 推進膠囊裡（新增 5 筆重疊）；改成只對 `Day Group 2` 自己的 `padding-top`，不動 Feed 共用的 `gap`，才不會波及其他手足節點。
...
- **定案形態全畫出，含深色（MJ-4／MJ-7／MJ-8）**：`cmp/Tab Bar` 旁 `cmp/Tab Bar · 四態對照（LS-120 R2）`（`Lo8Ae`，預設＋AX3 兩排）＋獨立頂層板 `cmp/Tab Bar · 四態對照 · 深色（LS-120 R3）`（`cKFJq`，比照既有深色板慣例設在板層級的 `theme:{mode:"dark"}`——**巢狀 theme 設在子 frame 上，資料層算得出深色 hex 但 `TakeScreenshot` 渲染引擎不吃，R3 新發現的 Pencil 限制**，深色排因此改成獨立頂層板而非 Lo8Ae 內的第三排）。三排×四欄共 12 個 instance，是驗收條件 1–2「有定案形態、全 app 一條規則」在四個狀態×三種主題都畫出來意義下的證據；override 配方＝每個 instance 的 `descendants` 顯式覆寫全部 4 顆 Indicator（fill/stroke/strokeWidth/effect）＋4 顆 Icon（fill/width/height）＋4 個 cell 的 `metadata.selected`——**選中片 `strokeWidth` 必須跟緊元件本體的值**（R3 元件改 1.5 後 12 個 demo instance 一度卡在硬寫的 1，R4 已同步改 1.5，見 MJ-8）。
...
- Pencil 實作細節（旋轉感知校準證據、preShift／padding-top 逐板精算值與踩雷記錄、MJ-6 標題對照表、MJ-7 深色板限制記錄、MJ-8～MN-11 處置）見 `design/littlesprout.pen` Handoff Notes `LuHbv`（LS-120／LS-120 R2／LS-120 R3／LS-120 R4 段；**R3 段的板高延伸數字已標記作廢，勿依其實作**）。
