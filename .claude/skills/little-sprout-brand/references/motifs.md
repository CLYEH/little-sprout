...
- **末張卡角托完全隱藏＝既定視覺，非殘留裝飾（LS-208 範圍 1 裁決）**：時間軸 5 個 Tab-root 板（`yXSht`／`a67Na`／`V4Ktak`／`lKoZG`／`A33la`，即「Tab Bar 全字級純 icon」節驗收判準列出的同一批板）Feed 內最後一張 `cmp/Card Diary` 的四顆角托，因該卡幾乎整張落在 Tab Bar 前緣淨空以下，被 Body（非 Feed 本身——Feed 一律 `clip:undefined`／未設）的 `clip:true` 邊界裁掉，逐板 4 顆共 20 筆完全不可見。**量法**：沿用 LS-201 VR R3 comment `1d0b97c2` 的定義——葉節點沿 `clip:true` 祖先鏈逐層相交後的可見矩形，可見區為空即完全隱藏（與 `corner_anchor` 掃描量的角托錨點錯位是兩件事：這裡角托座標本身沒有問題，只是被裁到看不見）。**裁決**：接受為既定視覺，不統一加板高——這是「捲動列表最後一張卡只露出頂緣一小截、暗示可繼續往下滑」既有版面節奏的必然結果，角托完全不可見時對使用者觀感無差異（下一張卡片完整可見時角托正常顯示）。節點清單（末張 `cmp/Card Diary` instance，每板皆同名 4 顆角托）：`MYxf1`／`DjW91,JgUK8,mlORb,atAkU`（`yXSht`）、`zfSO1`（`a67Na`）、`bOKfx`（`V4Ktak`）、`uvL4p`（`lKoZG`）、`Z9Xuy`（`A33la`）；與 LS-96 池項 `1d0b97c2` 記錄的 17 筆屬同一類缺陷（量級相符，因原始量測腳本 `LS-201-vr3-probe.py` 已刪除無法逐筆對帳出差額 3 筆）。完整推導與訂正過程見 `design/littlesprout.pen` Handoff Notes `yec61/Djv58`（LS-208）。此為終局裁決，之後遇到同一批節點不需再重新量測討論。
