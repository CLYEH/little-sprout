...
## Tab Bar 全字級純 icon（LS-120，R3 修訂——取代 R1／R2 描述）
...
- **尺寸**：`$icon-tab-md`（unselected，26／AX3 40）、`$icon-tab-lg`（selected，32／AX3 48）——比一般 `$icon-md`/`$icon-lg` 大一階，理由：導覽用的圖示不是「一般圖示」，拿掉文字後空間要還給 icon。indicator padding 固定 `$sp-tight`（6，不隨 AX3 放大，見 tokens.md ④）。選中 Indicator＝icon+12：預設 32+12=**44**（恰為觸控下限，膠囊本身即合格目標）、AX3 48+12=**60**。外層膠囊：預設 361×64 不變（content area 64−12=52，52−44=8=`$sp-label`，殘量落在節奏上）；AX3 361×**88**（content area 88−12=76，76−60=16、每側 8=`$sp-label`，殘量落在節奏上）。
- **y 位移**：Tab-root 板既有公式「y＝板高−98」（34pt 底部安全邊界＋64pt 膠囊高，見下「時間軸建立入口」條）不變；AX3 板「y＝板高−122」（34＋88）不變——**這兩條公式本身從未錯，R3 修的是公式的輸入（板高），不是公式本身**，見下「板高＝內容高」段。
- **捲動內容底部 inset 契約**（實作契約，寫給 ios-dev）：Tab-root 畫面捲動容器的 `safeAreaInset(edge:.bottom)`／`contentInset.bottom` ＝ `34 ＋ 膠囊高`——預設 **98**、AX3 **122**。這是給 ios-dev 的**實作契約值**，不是 .pen 靜態稿用來清空文字重疊的機制（R2 曾誤用一個等於契約值的 spacer 插在 feed 中間，見下一條的教訓）。
- **板高＝內容高，Tab Bar 淨空是內容的一部分（R3，修正 R2 回歸）**：motifs.md 既有規則「畫面宣告高＝內容高」對 Tab-root 板必須把 Tab Bar 的 34pt＋膠囊高也算進「內容」——R1／R2 的 6 張板（`yXSht`／`a67Na`／`V4Ktak`／`A33la`／`lKoZG`／`HLXo3`）宣告高只抄了 852／1500 的裝置慣例值，沒有反推「這份 feed 內容＋Tab Bar 淨空實際需要多高」，導致捲動 feed 的自然內容（日記卡片簽名列、日期章、多寶貝 caption）與膠囊在同一個 852/1500 的視窗裡打架。**驗收判準（R3，取代「0 重疊」這種模糊講法）**：板內任何 `text`／`icon` 葉節點的絕對 Y 範圍與 Tab Bar 矩形的 Y 範圍零交集（entirely above 或 entirely past）。**修法**：不是在 feed 中間插空白 spacer（那是回歸，見下一條），是把板高**延伸**到內容實際需要的高度，Tab Bar 既有的 y 公式原樣套用，自然清空：yXSht 852→896、a67Na 852→876、V4Ktak／A33la 852→888、lKoZG 1500→1600、HLXo3 1500→1648（精確違反量與 margin 見 Notes `LuHbv` R3 · BL-4 逐板結果段）。Body／Home Indicator Area 是既有 flex 結構（`fill_container`／固定 34），板高改變後自動跟隨，不需手動調整。
  - **反面教材（R2 回歸，不要重蹈）**：把一個等於「契約值」的空白 spacer 插在 feed 內容中間（例如 Cards 或 Day Group 之間），表面上「清空了重疊」，實際上（a）在兩則本來連續的內容之間製造一個視覺洞（yXSht 217.6pt＝25.5% 螢幕），（b）把洞後面的內容整個推出板外被 `clip:true` 吃掉，稿面上等於「刪除了這則日記」。「捲動內容底部 inset」在定義上只能是**內容最後面**的一段，不能是中間的洞。
...
- Pencil 實作細節（BL-4 逐板精確違反量／margin、MJ-6 標題對照表、MJ-7 深色板限制記錄、MN-5～9 處置）見 `design/littlesprout.pen` Handoff Notes `LuHbv`（LS-120／LS-120 R2／LS-120 R3 段）。
