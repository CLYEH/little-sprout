...
- **Feed 內最後一張帶角托的卡（`cmp/Card Album`）角托完全隱藏＝既定視覺，非殘留裝飾（LS-208 範圍 1 裁決；R2 依 merge-review comment e1e5157f M-A 訂正元件歸屬——原句誤植為 `cmp/Card Diary`，`cmp/Card Diary` 本身零角托，已訂正如下）**：時間軸 5 個 Tab-root 板（`yXSht`／`a67Na`／`V4Ktak`／`lKoZG`／`A33la`；「Tab Bar 全字級純 icon」節驗收判準列出的六板另含 `HLXo3`，其 Card Album 已於 LS-120 R6 刪除、不在此列）Feed 內最後一張帶角托的卡 `cmp/Card Album`（其後還有一張 `cmp/Card Diary`，本身零角托，是 Feed 真正的最後一張卡）的四顆角托，因該卡幾乎整張落在 Tab Bar 前緣淨空以下，被 Body（非 Feed 本身——Feed 一律 `clip:undefined`／未設）的 `clip:true` 邊界裁掉，逐板 4 顆共 20 筆完全不可見。**量法**：沿用 LS-201 VR R3 comment `1d0b97c2` 的定義——葉節點沿 `clip:true` 祖先鏈逐層相交後的可見矩形，可見區為空即完全隱藏（與 `corner_anchor` 掃描量的角托錨點錯位是兩件事：這裡角托座標本身沒有問題，只是被裁到看不見）。**裁決**：接受為既定視覺，不統一加板高——這是「捲動列表最後一張卡只露出頂緣一小截、暗示可繼續往下滑」既有版面節奏的必然結果，角托完全不可見時對使用者觀感無差異（下一張卡片完整可見時角托正常顯示）。節點清單（Feed 內最後一張帶角托的 `cmp/Card Album` instance，`ref → bhroo`，每板皆同名 4 顆角托、`ref → GEBcf`）：`MYxf1`／`DjW91,JgUK8,mlORb,atAkU`（`yXSht`）、`zfSO1`（`a67Na`）、`bOKfx`（`V4Ktak`）、`uvL4p`（`lKoZG`）、`Z9Xuy`（`A33la`）；與 LS-96 池項 `1d0b97c2` 記錄的 17 筆屬同一類缺陷（量級相符，因原始量測腳本 `LS-201-vr3-probe.py` 已刪除無法逐筆對帳出差額 3 筆）。完整推導與訂正過程見 `design/littlesprout.pen` Handoff Notes `yec61/Djv58`（LS-208；R2 訂正見同節沿革句）。此為終局裁決，之後遇到同一批節點不需再重新量測討論。
...
- 每顆角托一顆，R＝角托邊長×3，opacity＝0.03＋0.50×lit(角座標)；lit 以 viewport 座標算（窗光在 55,17，橢圓 668×1150）。池深＝那個角的光強度——規則自己執行自己。lit() 在**設計時**對印品的參考位置求值，結果凍結為元件常數（例：`cmp/Card Album`／`cmp/Card Photo` 的 0.393／0.259／0.293／0.197）；捲動清單內的實例一律取元件常數、不隨 viewport 變，只有不捲動的畫面（歡迎頁家族）才對實際座標求值（LS-372）。
...
- **有左下角托的印品，白邊上靠左的文字列要避開 BL 池暗角**（LS-372）：判準是可驗的對比——「字起點 × 最低墨點（含拉丁下伸部，紙底 −8），在該卡 bottomLeading 池 α0 下對紙 ≥4.5:1」，不是幾何淨空。`cmp/Card Album`／`cmp/Card Photo` 的 Imprint Row 左右 padding `$sp-group` 12，字起點距紙緣 20（照片仍在 `print-edge` 8），與 `cmp/Card Diary` 的 `$inset-card` 20 同軸，整條 feed 文字軸在板 x＝44；右側 12 只為對稱。此內縮在該卡 α0 < 0.361 成立（最低墨點處：稿面元件池 0.293 → 5.08；歡迎頁 `.welcome` 0.36 → 4.51，只剩 0.001 餘裕、小於模型誤差，所以這兩張卡的池值必須改用元件常數，只內縮不改池值不得出貨）。**池 α 必須是每卡常數**（見上條「設計時求值、凍結為元件常數」）——捲動清單內的印品不得依 viewport 動態算 lit()。驗收以模擬器深色截圖實測為準，測試資料含拉丁下伸部名字。同 Lab Imprint 置中，是版面避開暗角、不改池子。只適用有 BL 角托（因而有 BL 池）的印品；對角 TL／BR 兩顆角托的印品（例：Food 04）沒有 BL 池，字照常貼紙緣 8。量法與逐帶數字見 `design/littlesprout.pen` Notes `kHDk4` LS-372 段。
