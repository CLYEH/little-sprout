...
- **後端 LS-90 對齊中**：現行部署的 `create_invite` 仍是 **8 碼／40 bit**（`supabase/migrations/20260823010000_join_approval.sql`；`supabase/tests/80_join_approval.sql` 釘死 `{8}`），LS-90 把它改成 6 碼。**LS-90 併入前設計稿不得改成 8 格**——稿上 06 家族的 6 格 3+3 就是定案形狀；併入前後端回 8 碼是後端還沒對齊，不是設計要改，ios-dev 對接以 LS-90 併入後的契約為準。
