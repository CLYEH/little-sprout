...
- icon 一律帶文字標籤；層級淺（首頁 2 步內到內容）；不用雙擊、長按等進階手勢。
  ...
  - **明文例外二（使用者 2026-09-02 16:00 裁決，LS-120）**：`cmp/Tab Bar` 全字級（含預設字級，不只 AX 字級）純 icon，不掛文字標籤。前提四條缺一不可：①四顆 icon（時間軸／相簿／寶貝／設定）語意必須自明、彼此剪影可辨——選型理由見 `motifs.md`「Tab Bar 全字級純 icon」；②選中態對比與形狀變化要強，非只換色（背景由透明變 `$surface` 實心圓角＋落影、icon 尺寸放大、色由次要轉主要，三個維度同時變化）；③VoiceOver `accessibilityLabel`（＝原中文標籤）與 AX 字級的 Large Content Viewer（長按彈出大字標籤，`accessibilityShowsLargeContentViewer`）必備，作為拿掉可見文字的交換條件，不得省略；④點擊目標 ≥44pt，AX3 下 icon 隨 `$icon-md`／`$icon-lg` token 放大且零溢出（AX3 膠囊改 361×80，見 motifs.md）。除此之外，全 app 其餘畫面「icon 一律帶文字標籤」硬約束不變——這個例外限定 `cmp/Tab Bar` 一個元件，不得比照推廣到其他 icon。設計稿見 `design/littlesprout.pen` Handoff Notes `LuHbv`（LS-120 段）。
