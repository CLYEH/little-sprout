...
- icon 一律帶文字標籤；層級淺（首頁 2 步內到內容）；不用雙擊、長按等進階手勢。
  ...
  - **明文例外二（使用者 2026-09-02 16:00 裁決，LS-120；R2 補第⑤前提＋修正②③）**：`cmp/Tab Bar` 全字級（含預設字級，不只 AX 字級）純 icon，不掛文字標籤。前提五條缺一不可：①四顆 icon（時間軸／相簿／寶貝／設定）語意必須自明、彼此剪影可辨——選型理由見 `motifs.md`「Tab Bar 全字級純 icon」；②選中態對比與形狀變化要強，非只換色，且**必須附量測數字**（R1「三個維度」的斷言被深色模式實測 1.140:1 反駁；R2 改為四個維度：背景 `$surface` 實心圓角＋落影、新增 1pt `$control-line` 描邊使背景維度本身達淺 3.249:1／深 3.527:1、icon 尺寸 26→32、色次要轉主要）；③VoiceOver `accessibilityLabel`（＝原中文標籤，掛在**可點的 cell**、非 icon 葉節點）與 `selected` trait 必備；AX 字級的 Large Content Viewer（長按彈出大字標籤）是**加強**，不是唯一文字路徑——長按本身是本硬約束「不用長按等進階手勢」禁止的手勢，只押寶在它身上等於文字路徑 100% 依賴一個被禁止的手勢，見第⑤條；④點擊目標 ≥44pt，AX3 下 icon 隨 `$icon-tab-md`／`$icon-tab-lg` token 放大且零溢出（AX3 膠囊改 361×88，見 motifs.md）；**⑤非手勢替代路徑（R2 新增，比照拖曳例外前提④同型）：四個 tab 對應的目的地畫面，display 標題＝該 tab 名稱**（時間軸／相簿／寶貝／設定），是不需要長按、不需要 VoiceOver 也能對照「這顆圖示是什麼」的路徑；Tab-root 畫面的硬規則，未來新增的畫面票必須遵守，不得自由發揮標題（現況：時間軸已符合，寶貝用「寶貝管理」非精確比對、記入 LS-96 池，相簿／設定畫面尚未存在）。除此之外，全 app 其餘畫面「icon 一律帶文字標籤」硬約束不變——這個例外限定 `cmp/Tab Bar` 一個元件，不得比照推廣到其他 icon。設計稿見 `design/littlesprout.pen` Handoff Notes `LuHbv`（LS-120／LS-120 R2 段）。
