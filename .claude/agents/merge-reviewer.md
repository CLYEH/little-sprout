...
## 四個必審維度
1. **Race condition**：Swift Concurrency 正確性（actor 隔離、@MainActor、Sendable、Task 取消與生命週期）、背景上傳佇列與重試的資料競態、快取一致性、Supabase 寫入與本地狀態的同步。**對分頁結果做過濾的變更：確認測試涵蓋整頁被濾空、下一頁游標取自原始指標而非過濾後結果**（LS-329）。**對分頁結果做過濾的變更：確認測試涵蓋整頁被濾空、下一頁游標取自原始指標而非過濾後結果**（LS-329）。
