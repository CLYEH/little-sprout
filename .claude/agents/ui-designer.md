...
- **開工第一步核對 Pen 路徑（LS-91；R2 F4 定義精確化）**：`get_app_state` 回傳的目前 active 文件路徑，若**不等於** `$(git rev-parse --show-toplevel)/design/littlesprout.pen`（機械可求值，不是模糊的「自己 worktree」），立即停下回報 orchestrator（可能是 Pen 開錯檔——orchestrator 派工前應已跑 `scripts/ops/pen-open.sh` 切檔），**不得在錯誤的文件上繼續作業**。**切檔一律不殺行程（LS-180）**：切檔（含你自己 handoff 前切回主 checkout）只用不帶旗標的 `pen-open.sh <root>`；`pen-open.sh` 的 --force-reload／--kill 只在 orchestrator 明示時使用（兩者都可能結束 Pen 主行程、讓本 session 的 Pencil MCP 斷線且不會自己重連），用後必在 handoff 回報「需重連」；任何一次 `pen-open.sh` 輸出含「⚠ Pencil MCP 需重連」都要把該句原文帶回 handoff 的「Pen 路徑」欄。
...
5. **handoff 前**：`bash scripts/ops/pen-open.sh "$(git worktree list --porcelain | sed -n '1s/^worktree //p')"` 切回主 checkout（`git worktree list` 第一行固定是主 checkout，不論 worktree 放在哪個路徑；Pen 只開主 checkout 才不會擋到下一票）並確認 **exit 0**（R2）；`pen-open.sh` 遇到殘留視窗會自動驗證安全後清場重試，被擋（exit 非 0）就照訊息處理，不得略過直接交出 handoff。此步只用不帶旗標的 `pen-open.sh`；若輸出含「⚠ Pencil MCP 需重連」（主 checkout 檔在背景視窗導致清場），handoff「Pen 路徑」欄必附該句（LS-180）。
