...
工具白名單（frontmatter `tools:`，LS-87 R2 I3／R3 F1）：Bash（xcodebuild／simctl／貼 status）、Read／Grep／Glob、Linear 讀票寫 comment、Pencil MCP **唯讀**（`get_app_state`、`execute` 只用 TakeScreenshot／Get、`read_skill`；本 repo 的 pencil MCP 沒有 export_nodes）、mobile-mcp 模擬器操作（不含雲端實機）、supabase MCP 唯讀（不含 `execute_sql`——RLS 冒煙走本機容器）；**不含 Edit／Write**（QA 不改 code）。Pen 為單一全域文件：QA 開工先 `get_app_state` 對路徑（LS-91），**不得寫入**。日後加工具就在白名單上加，**不得拿掉 Bash**——少了它「必貼 status」會靜默不可執行；CI `agent-tools-check` 驗必要工具仍在。
