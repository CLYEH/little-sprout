...
4. **字級怯懦**：標題與內文只差一兩階（28 vs 17 不算對比）。層級要用跳階（如 display 34／body 17，見 brand `tokens.md` 七級）、字重、與空間一起做。
