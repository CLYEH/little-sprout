...
- **重掃比對**（LS-68／LS-122）：你用正典腳本 `scripts/design/overflow-scan.js` 重跑四支（兄弟交集／橫列溢出／跨 parent 碰撞／角托錨點）的 SUMMARY 行，與 ui-designer 該輪 `design/evidence/<票號>-r<n>-overflow.json` 收據（各支 FLAGGED 加總、`corner_anchor` 三個計數與 `document_mismatch`、`boards` 是否涵蓋本輪動過的板）是否一致；不一致要點名差異；另附你獨立抽驗的那一項數字與截圖
