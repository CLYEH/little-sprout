...
#   (b) 含「已修」的行必須帶 7–40 位小寫 hex 的 commit SHA。
#   刻意全 body 掃、不辨「處置語境」（判語境太脆）：引用而非申報的行（「LS-138 已修」「不另立 LS-96 池項」）會誤中，
#   由 agent 改寫措辭避開字樣或補 id；同一行多個 hex token 時任一可驗即過（一行同時寫 SHA 與 comment id 是常態）。
...
#   皆為祖先；已在 base 上的 commit 也是祖先——盲區，COLLABORATION §7）。無 LINEAR_API_KEY → 印「反查略過」只驗格式＋git
...
#   --verify 反查不到）；2＝參數／分支／環境錯誤（fail closed）。
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  case "$line" in
    *已修*)
      shas=$(hex_tokens "$line" 7 40)
      if [ -z "$shas" ]; then
        echo "✗ pr-body-check：第 ${n} 行含「已修」，但沒有 commit SHA（7–40 位 hex）：" >&2
        echo "    | ${line}" >&2
        bad_b=1
      else
...
if [ "$bad_b" -eq 1 ]; then
  echo "  每條「已修」必附 commit SHA（＋檔案:行號）寫在同一行，如「已修 \`7c2ff80\` \`Foo.swift:12\`」；沒有 SHA 的「已修」視為未修。" >&2
fi
...
# ---- --verify (b)：候選 SHA 至少一個存在且為 HEAD 祖先（需在 repo 內執行）----
fail=0
while read -r ln toks; do
  [ -n "$ln" ] || continue
  ok=
  for t in $(printf '%s' "$toks" | tr , ' '); do
    if git cat-file -e "${t}^{commit}" 2>/dev/null && git merge-base --is-ancestor "$t" HEAD 2>/dev/null; then ok=$t; break; fi
  done
  if [ -n "$ok" ]; then
