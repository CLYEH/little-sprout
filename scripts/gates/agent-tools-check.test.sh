...
# LS-327（LS-96 池項 dfff5ab3，來源 LS-321 R3 designer 實測）：ui-designer／visual-reviewer 正文另須含
# instance 覆寫子節點的擷取限制句（Export／TakeScreenshot 子節點 id 會回元件預設值，一律整板截圖再依
# Get 絕對座標裁切）；併進兩份合法樣本
...
SELFCHECK='handoff 加一段標題 `## 自檢（依 docs/REVIEW-RUBRIC.md）`，逐條附證據。'
IOS_BODY="${IOS_BODY} ${SELFCHECK}"
# LS-333（池項 5ec3954d①，來源 LS-315 QA 實測）：qa 正文另須含兩句陷阱原文；併進 QA_BODY
...
reset; expect 0 '⑤ 三份正文含字樣 → 印「正文含」、通過（76 條）' 'ios-dev.md：正文含「supabase-lock.sh --hold」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉓ dead-code-sweeper 正文含禁派 fork 句 → 印「正文含」（總數 76 條）' 'dead-code-sweeper.md：正文含「禁派 fork」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉕ merge-reviewer 正文含 ubuntu ≥10 次句 → 印「正文含」（總數 76 條）' 'merge-reviewer.md：正文含「shell 自測在 ubuntu:24.04 通道跑 ≥10 次」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉖ merge-reviewer 正文含兩句 → 印「正文含」（總數 76 條）' 'merge-reviewer.md：正文含「UITest mutation 重放要看失敗點／時間軸是否隨 mutation 改變」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉗ 三份正文含 scripts/ops/ci-wait.sh → 印「正文含」（總數 76 條）' 'ios-dev.md：正文含「scripts/ops/ci-wait.sh」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉘ ios-dev 正文含「實作新畫面必逐條對 Notes「畫面級屬性」並在 handoff 勾選」→ 印「正文含」（總數 76 條）' 'ios-dev.md：正文含「實作新畫面必逐條對 Notes「畫面級屬性」並在 handoff 勾選」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉙ ios-dev 正文含 push-gate 進度句 → 印「正文含」（總數 76 條）' 'ios-dev.md：正文含「逾時被背景化就前景重跑 push，快取秒過」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉛ 六份正文含 mcp__linear__* 備援句 → 印「正文含」（總數 76 條）' 'ios-dev.md：正文含「並在 handoff 註明走備援」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉜ ui-designer／visual-reviewer 正文含 instance 覆寫子節點句 → 印「正文含」（總數 76 條）' 'ui-designer.md：正文含「instance 內覆寫的子節點不可直接」' 'visual-reviewer.md：正文含「instance 內覆寫的子節點不可直接」'
...
out="$(bash "$mut" "$agents" 2>&1)"; got=$?
if [ "$got" -eq 0 ] && ! grep -qF 'instance 內覆寫的子節點不可直接' <<<"$out"; then
  ok '㉜ mutant（ui-designer）：拿掉規則後「缺 instance 覆寫子節點句」的負樣本變綠'
else
  echo "✗ ㉜ mutant（ui-designer instance 覆寫子節點句）應 exit 0 且不印該字樣（實得 ${got}）" >&2; printf '%s\n' "$out" | sed 's/^/    /' >&2; fail=1
fi
reset; mk visual-reviewer NONE "${KILL} ${NOFORK254} ${EDITID} ${CIWAIT} ${LINEARFALLBACK}"
out="$(bash "$mut" "$agents" 2>&1)"; got=$?
if [ "$got" -eq 0 ] && ! grep -qF 'instance 內覆寫的子節點不可直接' <<<"$out"; then
  ok '㉜ mutant（visual-reviewer）：拿掉規則後「缺 instance 覆寫子節點句」的負樣本變綠'
else
  echo "✗ ㉜ mutant（visual-reviewer instance 覆寫子節點句）應 exit 0 且不印該字樣（實得 ${got}）" >&2; printf '%s\n' "$out" | sed 's/^/    /' >&2; fail=1
fi
reset

# ---- ㉝ LS-333（源自 LS-313 R2／R3，池項 f99a2749）：ios-dev 正文須含「先前景跑 push-gate.sh 建快取、快取命中後
#      再 git push」——缺即紅、其餘句子齊全不救；同一 mutant 下負樣本變綠 ----
reset; expect 0 '㉝ ios-dev 正文含 push-gate 快取前置句 → 印「正文含」（總數 76 條）' 'ios-dev.md：正文含「先前景跑 push-gate.sh 建快取、快取命中後再 git push」' '正文必含字樣 76 條）'
...
reset; expect 0 '㉞ ios-dev 正文含 push 成功判準句 → 印「正文含」（總數 76 條）' 'ios-dev.md：正文含「不看背景通知的 exit code」' '正文必含字樣 76 條）'
...
if [ "$got" -eq 0 ] && ! grep -qF '不看背景通知的 exit code' <<<"$out"; then
  ok '㉞ mutant：拿掉規則後「缺 push 成功判準句」的負樣本變綠'
else
  echo "✗ ㉞ mutant（push 成功判準句）應 exit 0 且不印該字樣（實得 ${got}）" >&2; printf '%s\n' "$out" | sed 's/^/    /' >&2; fail=1
fi
reset

# ---- ㉟ LS-333（池項 5ec3954d①，來源 LS-315 QA 實測）：qa 正文須含「WDA 會與新 scene 競態、app 被背景化」——
#      缺即紅、其餘句子齊全不救；同一 mutant 下負樣本變綠 ----
reset; expect 0 '㉟ qa 正文含 WDA 競態陷阱句 → 印「正文含」（總數 76 條）' 'qa.md：正文含「WDA 會與新 scene 競態、app 被背景化」' '正文必含字樣 76 條）'
...
if [ "$got" -eq 0 ] && ! grep -qF 'WDA 會與新 scene 競態、app 被背景化' <<<"$out"; then
  ...
  echo "✗ ㉟ mutant（WDA 競態陷阱句）應 exit 0 且不印該字樣（實得 ${got}）" >&2; printf '%s\n' "$out" | sed 's/^/    /' >&2; fail=1
fi
reset

# ---- ㊱ LS-333：qa 正文須含「鍵盤彈出時會攔截下層按鈕的點擊」——缺即紅、其餘句子齊全不救；同一 mutant 下負樣本變綠 ----
reset; expect 0 '㊱ qa 正文含鍵盤攔截陷阱句 → 印「正文含」（總數 76 條）' 'qa.md：正文含「鍵盤彈出時會攔截下層按鈕的點擊」' '正文必含字樣 76 條）'
...
if [ "$got" -eq 0 ] && ! grep -qF '鍵盤彈出時會攔截下層按鈕的點擊' <<<"$out"; then
  ...
  echo "✗ ㊱ mutant（鍵盤攔截陷阱句）應 exit 0 且不印該字樣（實得 ${got}）" >&2; printf '%s\n' "$out" | sed 's/^/    /' >&2; fail=1
fi
reset
...
#      mutant 下負樣本變綠 ----
reset; expect 0 '㊲ merge-reviewer 正文含 rubric 引用句 → 印「正文含」（總數 76 條）' 'merge-reviewer.md：正文含「四維度檢查項的單一來源是 `docs/REVIEW-RUBRIC.md`」' '正文必含字樣 76 條）'
...
reset; expect 0 '㊶ ios-dev 正文含自檢段標題句 → 印「正文含」（總數 76 條）' 'ios-dev.md：正文含「## 自檢（依 docs/REVIEW-RUBRIC.md）」' '正文必含字樣 76 條）'
...
reset; mk ios-dev "$IOS_TOOLS" "$IOS_MINUS_SELFCHECK"
out="$(bash "$mut" "$agents" 2>&1)"; got=$?
if [ "$got" -eq 0 ] && ! has "$out" '## 自檢（依 docs/REVIEW-RUBRIC.md）'; then
  ok '㊶ mutant：拿掉規則後「缺自檢段標題句」的負樣本變綠'
else
  echo "✗ ㊶ mutant（自檢段標題句）應 exit 0 且不印該字樣（實得 ${got}）" >&2; printf '%s\n' "$out" | sed 's/^/    /' >&2; fail=1
fi
reset

# ---- ㊷ LS-352：merge-reviewer 正文須含「自檢已宣告通過的項只抽驗、不重寫」——缺即紅、其餘句子齊全不救；同一 mutant 下負樣本變綠 ----
reset; expect 0 '㊷ merge-reviewer 正文含自檢抽驗句 → 印「正文含」（總數 76 條）' 'merge-reviewer.md：正文含「自檢已宣告通過的項只抽驗、不重寫」' '正文必含字樣 76 條）'
...
reset; mk merge-reviewer "Bash, Read, Grep, Glob, ${LINEAR3}" "$MR_MINUS_SELFCHECKSAMPLE"
out="$(bash "$mut" "$agents" 2>&1)"; got=$?
if [ "$got" -eq 0 ] && ! has "$out" '自檢已宣告通過的項只抽驗、不重寫'; then
