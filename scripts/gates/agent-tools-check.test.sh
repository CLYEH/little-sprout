...
#   R1.5）；㊶ ios-dev 正文須含 handoff 自檢段標題——缺即紅、其餘句子齊全不救；同一 mutant 下負樣本變綠；正文必含
#   字樣總數 74→75。
set -uo pipefail
...
reset; expect 0 '⑤ 三份正文含字樣 → 印「正文含」、通過（75 條）' 'ios-dev.md：正文含「supabase-lock.sh --hold」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉓ dead-code-sweeper 正文含禁派 fork 句 → 印「正文含」（總數 75 條）' 'dead-code-sweeper.md：正文含「禁派 fork」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉕ merge-reviewer 正文含 ubuntu ≥10 次句 → 印「正文含」（總數 75 條）' 'merge-reviewer.md：正文含「shell 自測在 ubuntu:24.04 通道跑 ≥10 次」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉖ merge-reviewer 正文含兩句 → 印「正文含」（總數 75 條）' 'merge-reviewer.md：正文含「UITest mutation 重放要看失敗點／時間軸是否隨 mutation 改變」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉗ 三份正文含 scripts/ops/ci-wait.sh → 印「正文含」（總數 75 條）' 'ios-dev.md：正文含「scripts/ops/ci-wait.sh」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉘ ios-dev 正文含「實作新畫面必逐條對 Notes「畫面級屬性」並在 handoff 勾選」→ 印「正文含」（總數 75 條）' 'ios-dev.md：正文含「實作新畫面必逐條對 Notes「畫面級屬性」並在 handoff 勾選」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉙ ios-dev 正文含 push-gate 進度句 → 印「正文含」（總數 75 條）' 'ios-dev.md：正文含「逾時被背景化就前景重跑 push，快取秒過」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉛ 六份正文含 mcp__linear__* 備援句 → 印「正文含」（總數 75 條）' 'ios-dev.md：正文含「並在 handoff 註明走備援」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉜ ui-designer／visual-reviewer 正文含 instance 覆寫子節點句 → 印「正文含」（總數 75 條）' 'ui-designer.md：正文含「instance 內覆寫的子節點不可直接」' 'visual-reviewer.md：正文含「instance 內覆寫的子節點不可直接」'
...
reset; expect 0 '㉝ ios-dev 正文含 push-gate 快取前置句 → 印「正文含」（總數 75 條）' 'ios-dev.md：正文含「先前景跑 push-gate.sh 建快取、快取命中後再 git push」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉞ ios-dev 正文含 push 成功判準句 → 印「正文含」（總數 75 條）' 'ios-dev.md：正文含「不看背景通知的 exit code」' '正文必含字樣 75 條）'
...
reset; expect 0 '㉟ qa 正文含 WDA 競態陷阱句 → 印「正文含」（總數 75 條）' 'qa.md：正文含「WDA 會與新 scene 競態、app 被背景化」' '正文必含字樣 75 條）'
...
reset; expect 0 '㊱ qa 正文含鍵盤攔截陷阱句 → 印「正文含」（總數 75 條）' 'qa.md：正文含「鍵盤彈出時會攔截下層按鈕的點擊」' '正文必含字樣 75 條）'
...
#      mutant 下負樣本變綠 ----
reset; expect 0 '㊲ merge-reviewer 正文含 rubric 引用句 → 印「正文含」（總數 75 條）' 'merge-reviewer.md：正文含「四維度檢查項的單一來源是 `docs/REVIEW-RUBRIC.md`」' '正文必含字樣 75 條）'
...
reset; expect 0 '㊶ ios-dev 正文含自檢段標題句 → 印「正文含」（總數 75 條）' 'ios-dev.md：正文含「## 自檢（依 docs/REVIEW-RUBRIC.md）」' '正文必含字樣 75 條）'
