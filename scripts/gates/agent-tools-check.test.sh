...
# LS-346（來源 LS-343）：qa 正文另須含字級矩陣句（比預設小的字級也要驗，不是只驗放大端）；併進 QA_BODY
FONTMATRIX='字級矩陣至少含 xSmall、預設、AX3。'
QA_BODY="${QA_BODY} ${FONTMATRIX}"
...
reset; expect 0 '㊲ merge-reviewer 正文含分頁過濾游標句 → 印「正文含」（總數 75 條）' 'merge-reviewer.md：正文含「下一頁游標取自原始指標而非過濾後結果」' '正文必含字樣 75 條）'
...
reset; expect 0 '㊶ qa 正文含字級矩陣句 → 印「正文含」（總數 75 條）' 'qa.md：正文含「字級矩陣至少含 xSmall、預設、AX3」' '正文必含字樣 75 條）'
...
if [ "$got" -eq 0 ] && ! grep -qF '字級矩陣至少含 xSmall、預設、AX3' <<<"$out"; then
  ...
  echo "✗ ㊶ mutant（字級矩陣句）應 exit 0 且不印該字樣（實得 ${got}）" >&2; printf '%s\n' "$out" | sed 's/^/    /' >&2; fail=1
fi
reset
