...
# 特例的設計意圖矛盾（設計意圖是一個畫面一行、差異記在該行欄位裡）。`canonicalize_screen_name()` 剝掉板名的群組
# 前綴（`A11y`／`Stress` 這類衍生群組與其對應基準群組同編號即歸併）與已知裝飾字樣（`DERIVED_SUFFIX_RE`）算出
# canonical_id：有板編號（`01`／`04b`／`06a`）的畫面只認編號本身，沒有編號的名稱型畫面退回去裝飾後的描述文字
# （LS-251 實測：30 個變體板 → 8 個正典畫面）。`screen_attr_missing()` 對每個正典畫面只要任一 member 變體板名
# 出現在「畫面級屬性」段之後即算放行。
#
...
# 的正典畫面而非抓出這個打字錯，見 design_notes_check.py 檔頭；(6) 只驗變體板名子字串出現在段落之後的文字裡，不驗
# 「這一列真的是這塊板的列」——板名恰好出現在段落內其他畫面的敘述文字裡也會被誤判命中（誤放行方向，不誤擋；R2
# 起同一正典畫面本來就允許任一變體命中，這個盲區只影響「命中的是不是同一正典畫面內的敘述」這個更細的判斷）。
#
