...
已知限制（merge-review R1 informational，記錄不修，理由見 comment `b212dd78`；N6(a) 於 LS-292 隨
`BOLD_ONLY_RE` 放寬同步改寫，限制本身仍在，只是形狀變了；N6(b) 邏輯不受影響）：
  - N6(a) 段落中間出現「整行粗體（可選接括號附註／冒號，LS-292 起）且不接其他正文」的行仍會被判定為
    標題、提早結束段落，其後列項不檢查——`is_heading_line` 對粗體行的判定沒有排除段落內部的強調用語；
    LS-292 放寬 `BOLD_ONLY_RE` 讓段落標題可接「（附註）：」形狀後，這條限制同樣涵蓋「粗體＋括號附註／
    冒號」形狀的段落內強調用語（例如 `**注意**（僅供參考）：` 若出現在段落中間仍會被誤判為標題）。
...
# LS-292：粗體標題後可接可選的括號附註（全形（）／半形 ()）與可選冒號（全形：／半形:），其後不得
# 有其他文字——`**已驗證**（逐項對應票文驗收）：`／`**已驗證**：` 算標題；`**已驗證** 全部 14 條綠`
# （粗體後接非括號正文）仍不算（維持 N6(a) 語意，見檔頭已知限制段）。
...
def main(argv):
    ...
    for a in it:
        ...
        path = a
    if path is None:
        fail("用法：handoff_evidence_check.py <handoff.md> [--repo <dir>] [--ref <sha>]")

    ok = run(path, repo, ref)
