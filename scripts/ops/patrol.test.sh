...
# ㉜m／㉜m2（LS-330，範圍 3；LS-96 池項 `cd0e2847`(2)；merge-review R1 m2 修正）：patrol.sh:1055-1062
#     written_at 的「無 jq → grep 備援」分支先前零測試覆蓋（㉜a-l 全程式跑在有 jq 的機器上，只驗過 jq
#     分支）。這裡把 PATH 換成「複製一份、唯獨排掉 jq」的替身目錄——只把 jq 移出 PATH 不夠：只在較前面
#     的目錄擺一個不可執行的同名檔，bash 的 command -v 會略過它繼續往後面找到真 jq（LS-330 實測確認）；
#     只砍掉 jq 所在的那一個 PATH 目錄也不夠，那個目錄在不同機器上可能與 git／grep／sed 等其他必要工具
#     共用（如 Ubuntu 常見的 /usr/bin）。替身目錄完整複製「當下 PATH 每個目錄底下可解析到的每個檔名」
#     但唯獨跳過 jq，逼真的讓 `command -v jq` 在 patrol.sh 的子行程裡找不到，其餘工具（git／sed／grep／
#     date／mktemp…）都還在。
#     R1 m2（merge-review 實測）：㉜m 原本用 `usage_mk 98`（`printf` 緊湊單行）建快取，跟 ㉜l 的
#     `usage_mk 98 | jq .`（pretty-print，`:` 後多一個空白）不是同一種格式——patrol.sh:1059 的 grep 備援
#     舊版是 `'"written_at":[0-9]+'`（不容冒號後空白），對 pretty-print 快取會判成 written_at 抓不到、
#     誤報「JSON 不可解析或必要欄位缺失」，跟 ㉜m 註解宣稱的「與 ㉜l 同一組資料、兩條分支等價」不符
#     （㉜m 只驗了緊湊格式）。改採「修 grep 正則＋補 pretty-print 變體」：grep 備援放寬成
#     `'"written_at"[[:space:]]*:[[:space:]]*[0-9]+'`（sed 同步放寬），written_at 是頂層純量、pretty-print
#     不會把它拆到下一行（跟 usage_extract 要處理的巢狀物件展開不同），純粹是冒號後空白的問題；㉜m 保留
#     緊湊格式（驗 grep 備援的基本情況），新增 ㉜m2 用 pretty-print 格式（驗 grep 備援真的跟 ㉜l 同一種
#     輸入格式、算出同結果，不是巧合過關）。
if command -v jq >/dev/null 2>&1; then
  ...
  if PATH="$nojq_dir" command -v jq >/dev/null 2>&1; then
    ...
    has   '㉜m PATH 無 jq（grep 備援分支，緊湊 JSON）仍解析出 98% → 97 級警示' "$brief32m" '⚠ [用量] 週用量 98%（重置 '
    ...
    usage_mk 98 "$u_now" | jq . > "$work/usage-m2.json"
    brief32m2="$(PATH="$nojq_dir" PATROL_USAGE_FILE="$work/usage-m2.json" bash "$patrol" --repo "$repo" --no-pr --no-fetch --brief "$STALE" 2>&1)"
    has   '㉜m2 PATH 無 jq＋pretty-print JSON（同 ㉜l 的輸入格式）仍解析出 98% → 97 級警示，與 ㉜l 的 jq 分支真正同結果' "$brief32m2" '⚠ [用量] 週用量 98%（重置 '
    hasnt '㉜m2 不誤判「探針無資料」（R1 m2 修前的實際壞行為：written_at 抓不到）' "$brief32m2" '探針無資料'
    json32m2="$(PATH="$nojq_dir" PATROL_USAGE_FILE="$work/usage-m2.json" bash "$patrol" --repo "$repo" --no-pr --no-fetch --json "$STALE" 2>/dev/null)"
    jq_ok '㉜m2 --json usage.stale=false（pretty-print＋grep 備援，written_at 仍正確判出未過期）' "$json32m2" '.usage.stale == false'
  fi
else
  jq_skipped=$((jq_skipped + 1))
  echo "SKIP：本機無 jq，㉜m／㉜m2（PATH 去 jq 測 grep 備援）與既有 jq 分支已無差異可比，略過"
fi
...
# ---- ㉒ Pencil 連線探針（LS-180；MCP 訊號自 LS-308 起只 informational、不影響 pen-status.sh exit code）：只在有
#        design 分支 worktree 時跑 pen-status.sh；探針非 0（proc／path 造成）→ [Pencil] flag（human／brief／json 都帶、
#        指示先呼叫 get_app_state、失敗才 /mcp）；0 → 只印一行不標；沒有 design worktree → 略過、不呼叫探針；探針腳本
#        不存在 → 標 [Pencil] 說明 ----
...
printf '#!/bin/bash\necho "Pencil：行程 ✓（pid 1） · 路徑 ✗（pen CLI 讀不到 active 文件——CLI 未登入／desktop socket 連不上） · MCP：本 session 沒有 mcp-server 行程（尚未呼叫過 pencil MCP；下一次 mcp__pencil__* 呼叫會自動連上，失敗才 /mcp）"\nexit 1\n' > "$fake_ps_bad"
...
has   '㉒ human 印探針行' "$out22" '路徑 ✗'
has   '㉒ human 探針非 0 → 段內指示先呼叫 get_app_state、失敗才 /mcp' "$out22" '→ 設計票派工前先呼叫一次 mcp__pencil__get_app_state（懶連線會自動連上）；失敗才請使用者 /mcp'
...
has   '㉒ --brief flag 指示先呼叫 get_app_state、失敗才 /mcp' "$brief22" '設計票派工前先呼叫一次 mcp__pencil__get_app_state（懶連線會自動連上）；失敗才請使用者 /mcp'
json22="$(PATROL_PEN_STATUS_SH="$fake_ps_bad" bash "$patrol" --repo "$repo" --no-pr --no-fetch --json "$STALE" 2>/dev/null)"
jq_ok '㉒ --json：pencil.ran true、rc 1、line 含路徑 ✗、flags 恰一筆 [Pencil]' "$json22" \
  '.pencil.ran == true and .pencil.rc == 1 and (.pencil.line | test("路徑 ✗")) and ([.flags[] | select(startswith("[Pencil]"))] | length == 1)'
